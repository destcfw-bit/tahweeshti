...
const DEFAULT_PREFS = {
  ...
  simpleMode:true, soundEffects:true, onboardingDone:false
};
...
function humanDate(d){
  ...
  const relative=n===0?'اليوم':n===1?'غدًا':n===-1?'أمس':n>1&&n<=30?`بعد ${n} أيام`:n<0&&n>=-30?`منذ ${Math.abs(n)} أيام`:'';
  return `${relative?relative+' · ':''}${fmtDate(d)}`;
}

let audioCtx=null;
function audioReady(){
  if(!state.prefs.soundEffects)return null;
  try{audioCtx=audioCtx||new (window.AudioContext||window.webkitAudioContext)();if(audioCtx.state==='suspended')audioCtx.resume().catch(()=>{});return audioCtx}catch{return null}
}
function tone(freq,start,duration,volume=.025,type='sine',endFreq=0){
  const ctx=audioReady();if(!ctx)return;const o=ctx.createOscillator(),g=ctx.createGain(),now=ctx.currentTime+start;o.type=type;o.frequency.setValueAtTime(freq,now);if(endFreq)o.frequency.exponentialRampToValueAtTime(Math.max(40,endFreq),now+duration);g.gain.setValueAtTime(.0001,now);g.gain.exponentialRampToValueAtTime(volume,now+.012);g.gain.exponentialRampToValueAtTime(.0001,now+duration);o.connect(g).connect(ctx.destination);o.start(now);o.stop(now+duration+.02)
}
function playSound(kind='tap'){
  if(!state.prefs.soundEffects)return;
  if(kind==='nav'){tone(330,0,.09,.022,'sine',420);tone(520,.055,.11,.018,'sine',620)}
  else if(kind==='open')tone(420,0,.08,.018,'triangle',560);
  else if(kind==='close')tone(420,0,.09,.014,'triangle',300);
  else if(kind==='success'){tone(520,0,.09,.022,'sine',660);tone(760,.075,.14,.02,'sine',900)}
  else if(kind==='error')tone(210,0,.11,.018,'triangle',155);
  else tone(500,0,.055,.012,'sine',560)
}
...
function toast(msg, actionLabel='', actionFn=null){
  ...
  if(/تعذر|خطأ|غير صحيح|مرفوض/.test(String(msg)))playSound('error'); else if(/✅|تم |أهلًا|جاهز/.test(String(msg)))playSound('success');
...
function closeModal(){ playSound('close'); $('#modalRoot').classList.remove('open'); $('#modalRoot').setAttribute('aria-hidden','true'); document.body.style.overflow=''; state.modalContext=null; state.pendingReceipt=null; }

function validateShell(){
  const required=[
    '#splash','#authScreen','#app','#authForm','#authPin','#authPinConfirm','#loginSubmit',
    '#quickAddTop','#mobileAdd','#logoutBtn','#refreshBtn','#themeBtn','#privacyBtn','#globalSearchBtn',
...
function applyPrefs(){
  ...
  document.body.classList.toggle('simple-mode',!!state.prefs.simpleMode);
  document.body.classList.toggle('sounds-off',!state.prefs.soundEffects);
...
async function enterApp(){
  ...
  try{const sess=await api('session');state.accountId=sess.accountId||state.accountId;await fetchData();const s=await api('admin_status');state.admin=s;$('#authScreen').classList.add('hidden');$('#app').classList.remove('hidden');setMode();navigate('dashboard');maybeNotifyDue();handleQuickParam();maybeShowOnboarding();}
...
function navigate(view){
  const titles={dashboard:'الرئيسية',transactions:'الحركات',people:'الأشخاص',goals:'أهداف التحويش',planning:'تنظيم فلوسي',subscriptions:'الاشتراكات الشهرية',analytics:'التقارير',calendar:'المواعيد',settings:'الإعدادات'};
  const swap=()=>{state.view=view;$$('.view').forEach(v=>v.classList.toggle('active',v.id===`view-${view}`));$$('[data-nav]').forEach(b=>b.classList.toggle('active',b.dataset.nav===view));$('#pageTitle').textContent=titles[view]||'تحويشتي';renderCurrent()};
  if(state.view===view){swap();return}
  playSound('nav');document.body.classList.add('page-moving');
  if(document.startViewTransition){const vt=document.startViewTransition(swap);vt.finished.finally(()=>document.body.classList.remove('page-moving'))}
  else{const current=$('.view.active');current?.classList.add('view-leaving');setTimeout(()=>{swap();document.body.classList.remove('page-moving');$('.view.active')?.classList.add('view-entering');setTimeout(()=>$('.view.active')?.classList.remove('view-entering'),420)},150)}
  window.scrollTo({top:0,behavior:'smooth'})
}
function quickActionGrid(){return `<div class="quick-action-grid"><button class="quick-action qa-receivable mutating" data-act="new-entry" data-type="receivable"><span>💚</span><b>إلي عند شخص</b><small>فلوس لازم ترجعلي</small></button><button class="quick-action qa-payable mutating" data-act="new-entry" data-type="payable"><span>🔴</span><b>عليّ لشخص</b><small>فلوس لازم أدفعها</small></button><button class="quick-action qa-income mutating" data-act="new-entry" data-type="income"><span>💰</span><b>دخلني مبلغ</b><small>راتب، حوالة أو دخل</small></button><button class="quick-action qa-expense mutating" data-act="new-entry" data-type="expense"><span>💸</span><b>صرفِت مبلغ</b><small>مصروف أو عملية شراء</small></button></div>`}
function dashboardSearchHtml(){return `<div class="global-search-shell glass"><div class="global-search-title"><span>🔎</span><div><b>ابحث بكل شيء</b><small>شخص، حركة، هدف، اشتراك أو مبلغ</small></div></div><input id="globalSearchInput" class="global-search-input" placeholder="مثال: أحمد، الإنترنت، 25..." autocomplete="off"><div id="globalSearchResults" class="global-search-results hidden"></div></div>`}
function whatsTodayHtml(items){const list=items.filter(x=>x.days<=3).slice(0,5);if(!list.length)return `<div class="all-clear"><span>✓</span><div><b>ما عليك شيء عاجل اليوم</b><small>أمورك مرتبة 👌</small></div></div>`;return `<div class="today-actions">${list.map(x=>`<div class="today-action ${x.days<0?'late':''}"><div><b>${esc(x.title)}</b><small>${x.days<0?`متأخر ${Math.abs(x.days)} يوم`:x.days===0?'موعده اليوم':`موعده ${humanDate(x.date)}`}</small></div><strong data-money>${money(x.amount)}</strong></div>`).join('')}</div>`}


function renderDashboard(){
  const t=totals(),health=healthScore(),b=budgetUsage(),upcoming=upcomingItems(),due=upcoming.filter(x=>x.days<=14).slice(0,5),gs=goals().slice(0,3),hidden=new Set(state.prefs.hiddenCards||[]),healthLabel=health>=75?'ممتاز':health>=55?'متوازن':'يحتاج انتباه';
  $('#view-dashboard').innerHTML=`<section class="welcome-strip reveal"><div><span class="eyebrow">شو حاب تسجل؟</span><h3>اختار اللي صار وبنكملها معك خطوة خطوة</h3></div><button class="help-dot" data-help="الأخضر فلوس إلك، الأحمر فلوس عليك، الذهبي دخل، والأزرق مصروف.">?</button></section>${quickActionGrid()}${dashboardSearchHtml()}<article class="today-panel glass reveal"><div class="section-head"><div><span class="eyebrow">شو عليّ اليوم؟</span><h3>الأشياء اللي بدها انتباهك</h3></div><button class="btn ghost small" data-nav="calendar">كل المواعيد</button></div>${whatsTodayHtml(upcoming)}</article><div class="simple-money-grid ${hidden.has('stats')?'hidden':''}"><article class="simple-money-card mine"><span>💚</span><small>فلوس إلك</small><b data-money>${money(t.receivable)}</b></article><article class="simple-money-card owed"><span>🔴</span><small>فلوس عليك</small><b data-money>${money(t.payable)}</b></article><article class="simple-money-card net"><span>✨</span><small>الصافي</small><b class="${t.net<0?'neg':'pos'}" data-money>${money(t.net)}</b></article></div><div class="advanced-dashboard"><div class="hero-grid ${hidden.has('decision')?'hidden':''}"><article class="balance-card reveal"><div class="balance-shine"></div><span class="hero-label">وضعي اليوم</span><strong class="hero-amount ${t.net<0?'neg':''}" data-money>${money(t.net)}</strong><div class="hero-trend">إلي ${money(t.receivable)} · عليّ ${money(t.payable)}</div><div class="hero-actions"><button class="mini-action install" data-act="install-app">⇩ تنزيل التطبيق</button></div></article><article class="decision-card glass reveal delay-1"><div class="decision-icon">✦</div><span class="eyebrow">قراري اليوم</span><h3>${healthLabel}</h3><p>${esc(decisionText())}</p><div class="health-row"><span class="small-text">المؤشر ${health}/100</span><div class="health-meter"><span style="width:${health}%"></span></div></div></article></div><div class="stats-grid">${statCard('＋','gold','دخل هذا الشهر',monthTotals().income,'فلوس دخلت')}${statCard('−','blue','صرف هذا الشهر',monthTotals().expense,'فلوس طلعت')}${statCard('◎','purple','بالتوفير',savingsTotal(),'محافظ التوفير')}</div><div class="dash-grid"><article class="goal-preview glass ${hidden.has('goals')?'hidden':''}"><div class="section-head"><div><span class="eyebrow">أهداف التحويش</span><h3>تقدّمك</h3></div><button class="btn ghost small" data-nav="goals">عرض الكل</button></div><div class="stack">${gs.length?gs.map(goalMini).join(''):empty('ما في أهداف بعد','أضف أول هدف تحويش')}</div></article><article class="due-preview glass ${hidden.has('dues')?'hidden':''}"><div class="section-head"><div><span class="eyebrow">القادم</span><h3>مواعيد قريبة</h3></div><button class="btn ghost small" data-nav="calendar">المواعيد</button></div><div class="stack">${due.length?due.map(dueItem).join(''):empty('ما في مواعيد قريبة','أمورك مرتبة 👌')}</div></article></div><article class="section-card glass subscription-dashboard" style="margin-top:16px"><div class="section-head"><div><span class="eyebrow">الاشتراكات الشهرية</span><h3>مين راح يحول؟ وشو راح ينخصم؟</h3></div><button class="btn ghost small" data-nav="subscriptions">إدارة الاشتراكات</button></div>${subscriptionDashboardHtml()}</article><div class="grid-2" style="margin-top:16px"><article class="month-summary glass section-card ${hidden.has('summary')?'hidden':''}"><span class="eyebrow">هذا الشهر</span><h3>ملخص مالي</h3><p class="muted">${esc(monthSummaryText())}</p>${b.limit?`<div class="row"><span class="small-text">الميزانية</span><div class="progress spacer"><span style="width:${Math.min(100,b.pct)}%"></span></div><b>${Math.round(b.pct)}%</b></div>`:'<button class="btn secondary small mutating" data-act="budget-form">حدد ميزانية شهرية</button>'}</article><article class="achievement-preview glass section-card ${hidden.has('achievements')?'hidden':''}"><div class="section-head"><div><span class="eyebrow">إنجازات</span><h3>تقدّمك المالي</h3></div><button class="btn ghost small" data-nav="analytics">تفاصيل</button></div><div class="achievement-grid">${achievements().slice(0,4).map(a=>`<div class="achievement ${a.done?'done':''}"><b>${a.icon} ${esc(a.title)}</b><small>${esc(a.note)}</small></div>`).join('')}</div></article></div></div>`
}
...
function dueItem(x){const label=x.days<0?`متأخر ${Math.abs(x.days)} يوم`:humanDate(x.date);return `<div class="due-item"><i class="due-dot"></i><div><b>${esc(x.title)}</b><div class="tiny muted">${label}</div></div><strong class="${x.days<0?'neg':'gold'}" data-money>${money(x.amount)}</strong></div>`}
...
function renderPeople(){const list=peopleSummary();$('#view-people').innerHTML=`<div class="section-head"><div><span class="eyebrow">دفتر الأشخاص</span><h3>كل حساباتك حسب الشخص</h3><p>شوف بسرعة مين إلك عليه ومين إله عليك، وسجّل دفعة بدون تعقيد.</p></div><button class="btn secondary compact mutating" data-act="new-person">＋ شخص</button></div><div class="people-grid">${list.length?list.map(personCard).join(''):empty('ما في أشخاص بعد','أضف دين أو شخص جديد')}</div>`}
...
function renderSettings(){const hidden=new Set(state.prefs.hiddenCards||[]);$('#view-settings').innerHTML=`<div class="settings-grid"><article class="setting-card glass"><span class="eyebrow">سهولة الاستخدام</span><h4>كيف تحب تشوف البرنامج؟</h4><p>الوضع البسيط يخفي التفاصيل المتقدمة ويخلي أهم الأشياء قدامك.</p><div class="switch-row"><span>الوضع البسيط</span><button class="switch ${state.prefs.simpleMode?'on':''}" data-act="toggle-simple"></button></div><div class="row wrap" style="margin-top:12px"><button class="btn ghost small" data-act="replay-onboarding">❓ شرح البرنامج</button></div></article><article class="setting-card glass"><span class="eyebrow">الصوت والحركة</span><h4>تجربة حية</h4><p>أصوات خفيفة عند التنقل والحفظ، وانتقالات ناعمة بين الصفحات.</p><div class="switch-row"><span>أصوات التطبيق</span><button class="switch ${state.prefs.soundEffects?'on':''}" data-act="toggle-sounds"></button></div><div class="tiny muted">يمكنك إيقاف الصوت بأي وقت.</div></article><article class="setting-card glass"><span class="eyebrow">المظهر والخصوصية</span><h4>شكل التطبيق</h4><p>غيّر الوضع، أخفِ الأرقام، أو افتح وضع العرض فقط.</p><div class="switch-row"><span>الوضع الفاتح</span><button class="switch ${state.prefs.theme==='light'?'on':''}" data-act="toggle-theme"></button></div><div class="switch-row"><span>إخفاء المبالغ</span><button class="switch ${state.prefs.hideAmounts?'on':''}" data-act="toggle-privacy"></button></div><div class="switch-row"><span>عرض فقط</span><button class="switch ${state.prefs.viewOnly?'on':''}" data-act="toggle-viewonly"></button></div></article><article class="setting-card glass"><span class="eyebrow">الأمان</span><h4>القفل والحماية</h4><p>قفل تلقائي، تغيير رمز الدخول ورمز إدارة اختياري للحذف النهائي.</p><div class="row wrap"><button class="btn secondary small" data-act="autolock-form">⏱ القفل ${state.prefs.autoLockMinutes||0} د</button><button class="btn ghost small" data-act="change-pin">🔐 تغيير PIN</button><button class="btn ${state.admin.enabled?'secondary':'ghost'} small" data-act="admin-pin">🛡 ${state.admin.enabled?(state.admin.unlocked?'الإدارة مفتوحة':'فتح الإدارة'):'تفعيل رمز الإدارة'}</button></div></article><article class="setting-card glass"><span class="eyebrow">التنبيهات والتطبيق</span><h4>خليك متابع</h4><p>ثبّت التطبيق وفعّل تذكير المواعيد القريبة.</p><div class="row wrap"><button class="btn secondary small" data-act="install-app">⇩ تثبيت التطبيق</button><button class="btn ghost small" data-act="notifications">🔔 ${state.prefs.notifications?'مفعلة':'تفعيل التنبيهات'}</button></div></article><article class="setting-card glass"><span class="eyebrow">النسخ الاحتياطي</span><h4>بياناتك معك</h4><p>نزّل أو استرجع بيانات حسابك.</p><div class="row wrap"><button class="btn secondary small" data-act="export-backup">⬇ نسخة</button><button class="btn ghost small mutating" data-act="import-backup">⬆ استرجاع</button><button class="btn ghost small" data-act="emergency-backup">🆘 طوارئ</button></div></article><article class="setting-card glass advanced-setting"><span class="eyebrow">تخصيص الرئيسية</span><h4>شو بدك يظهر؟</h4>${[['decision','قراري اليوم'],['stats','البطاقات المالية'],['goals','الأهداف'],['dues','المواعيد'],['summary','ملخص الشهر'],['achievements','الإنجازات']].map(([k,l])=>`<div class="switch-row"><span>${l}</span><button class="switch ${!hidden.has(k)?'on':''}" data-act="toggle-card" data-card="${k}"></button></div>`).join('')}</article><article class="setting-card glass advanced-setting"><span class="eyebrow">السجل والصيانة</span><h4>تحكم كامل</h4><div class="row wrap"><button class="btn secondary small" data-act="trash">🗑 السلة</button><button class="btn ghost small" data-act="audit">🕘 سجل التغييرات</button></div></article></div><article class="section-card glass" style="margin-top:15px"><div class="row"><div><span class="eyebrow">تحويشتي</span><h3>الإصدار ${APP_VERSION}</h3><p class="muted small-text">مصمم ليكون مفهوم من أول استخدام · الدينار الأردني</p></div><span class="spacer"></span><div class="small-text muted">© Yahya Saeed</div></div></article>`}


/* Forms */
function entryTypeLabel(t){return ({receivable:'فلوس إلي عند شخص',payable:'فلوس عليّ لشخص',income:'دخلني مبلغ',expense:'صرفِت مبلغ'})[t]||'حركة'}
function entryFormHtml(e={}){const m=e.meta||{},type=e.type||'receivable',isDebt=['receivable','payable'].includes(type);return `<form id="entryForm" class="entry-wizard"><div class="wizard-step"><span>1</span><div><b>شو صار؟</b><small>اختار نوع الحركة</small></div></div><input type="hidden" name="type" value="${type}"><div class="type-choice-grid">${[['receivable','💚','إلي عند شخص'],['payable','🔴','عليّ لشخص'],['income','💰','دخلني مبلغ'],['expense','💸','صرفِت مبلغ']].map(([v,ic,l])=>`<button type="button" class="type-choice ${type===v?'active':''} type-${v}" data-entry-type="${v}"><span>${ic}</span><b>${l}</b></button>`).join('')}</div><div class="wizard-step"><span>2</span><div><b>التفاصيل الأساسية</b><small>بس المعلومات المهمة</small></div></div><div class="form-grid"><label class="field"><span class="person-label">${isDebt?'اسم الشخص':'الجهة / الوصف'}</span><input name="person" value="${esc(e.person||'')}" maxlength="120" placeholder="${isDebt?'مثال: أحمد':'مثال: راتب / سوبرماركت'}"></label><label class="field"><span>المبلغ بالدينار</span><input name="amount" type="number" min="0.001" step="0.001" inputmode="decimal" value="${e.amount??''}" placeholder="مثال: 25" required></label><label class="field"><span>التاريخ</span><input name="date" type="date" value="${e.date||today()}" required></label><label class="field debt-due ${isDebt?'':'soft-hidden'}"><span>موعد الدفع <button type="button" class="help-inline" data-help="اختياري. إذا حددته بنذكرك لما يقرب الموعد.">?</button></span><input name="dueDate" type="date" value="${m.dueDate||''}"></label><label class="field full"><span>ملاحظة <small>اختياري</small></span><textarea name="note" rows="2" maxlength="800" placeholder="مثال: باقي من فاتورة / دفعة أولى">${esc(e.note||'')}</textarea></label></div><details class="advanced-options" ${state.prefs.simpleMode?'':'open'}><summary>⚙️ خيارات إضافية <small>محفظة، طريقة الدفع، تصنيف، وسوم، صورة</small></summary><div class="form-grid advanced-fields"><label class="field"><span>التصنيف</span><select name="category">${categoryOptions(m.category||'عام')}</select></label><label class="field"><span>المحفظة <button type="button" class="help-inline" data-help="المحفظة تعني وين موجود المبلغ: كاش، بنك، CliQ أو توفير.">?</button></span><select name="walletId">${docOption(wallets(),m.walletId,'بدون محفظة')}</select></label><label class="field"><span>طريقة الدفع</span><select name="paymentMethod"><option value="">غير محدد</option>${paymentOptions(m.paymentMethod)}</select></label><label class="field"><span>الوسوم</span><input name="tags" value="${esc((m.tags||[]).join(', '))}" placeholder="شخصي، سيارة، مهم"></label><div class="upload-zone full"><input id="receiptFile" type="file" accept="image/jpeg,image/png,image/webp"><label for="receiptFile"><b>📸 ${e.receiptPath?'استبدال صورة الحوالة':'إضافة صورة حوالة / إيصال'}</b><small id="receiptName">JPG / PNG / WEBP — حتى 5MB</small></label><div class="upload-tools"><button type="button" class="btn ghost small" data-act="ocr-receipt">🔍 قراءة المبلغ من الصورة</button></div></div></div></details><div class="modal-actions">${e.id?`<button type="button" class="btn danger-soft mutating" data-act="delete-entry" data-id="${e.id}">حذف</button>`:''}<span class="spacer"></span><button type="button" class="btn ghost" data-close-modal>إلغاء</button><button class="btn primary mutating" type="submit">✓ حفظ ${entryTypeLabel(type)}</button></div></form>`}
function wireEntryForm(){const form=$('#entryForm');if(!form)return;const typeInput=form.querySelector('[name="type"]');form.querySelectorAll('[data-entry-type]').forEach(btn=>btn.onclick=()=>{playSound('tap');const type=btn.dataset.entryType;typeInput.value=type;form.querySelectorAll('[data-entry-type]').forEach(x=>x.classList.toggle('active',x===btn));const debt=['receivable','payable'].includes(type);form.querySelector('.person-label').textContent=debt?'اسم الشخص':'الجهة / الوصف';form.querySelector('[name="person"]').placeholder=debt?'مثال: أحمد':'مثال: راتب / سوبرماركت';form.querySelector('.debt-due')?.classList.toggle('soft-hidden',!debt);form.querySelector('button[type="submit"]').textContent=`✓ حفظ ${entryTypeLabel(type)}`});const f=$('#receiptFile');if(f)f.onchange=ev=>{state.pendingReceipt=ev.target.files[0]||null;$('#receiptName').textContent=state.pendingReceipt?`${state.pendingReceipt.name} — ${(state.pendingReceipt.size/1024/1024).toFixed(2)}MB`:'بدون ملف'}}
function openEntryForm(id='',preset={}){if(!guardMutation())return;const e=id?state.entries.find(x=>x.id===id):{type:preset.type||'receivable',amount:preset.amount||'',person:preset.person||'',date:preset.date||today(),note:preset.note||'',meta:preset.meta||{}};state.editingEntryId=id||null;state.pendingReceipt=null;openModal(id?'تعديل الحركة':entryTypeLabel(e.type),id?'عدّل اللي بدك إياه':'إضافة سهلة خطوة بخطوة',entryFormHtml(e),{context:{kind:'entry',id}});setTimeout(wireEntryForm,0)}
function openQuickChoice(){if(!guardMutation())return;openModal('شو صار؟','إضافة حركة',`<div class="quick-choice-modal">${quickActionGrid()}<div class="smart-quick"><b>أو اكتبها بطريقتك</b><div class="smart-row"><input id="smartInput" class="smart-input" placeholder="مثال: أحمد أخذ 25 اليوم"><button class="btn secondary" data-act="voice-smart">🎙️</button><button class="btn primary" data-act="parse-smart">✨ فهمها</button></div></div></div>`,{size:'wide'})}
...
function openPersonDetail(name){const p=peopleSummary().find(x=>x.name===name);if(!p)return;const prof=p.profile?.data||{},es=allEntries().filter(e=>e.person===name).sort((a,b)=>String(b.date).localeCompare(String(a.date))),sentence=p.net>0?`💚 ${name} لازم يحولك ${money(p.net)}`:p.net<0?`🔴 إنت لازم تدفع لـ ${name} ${money(Math.abs(p.net))}`:`✅ حسابك مع ${name} مسكّر`;openModal(name,'حساب الشخص',`<div class="person-status-sentence ${p.net>0?'mine':p.net<0?'owed':'settled'}">${sentence}</div><div class="detail-grid"><div class="detail-box"><span>فلوس إلك عنده</span><b class="pos">${money(p.receivable)}</b></div><div class="detail-box"><span>فلوس إله عندك</span><b class="neg">${money(p.payable)}</b></div><div class="detail-box"><span>الصافي</span><b>${money(Math.abs(p.net))}</b></div></div>${prof.note?`<div class="insight" style="margin-top:12px">📝 ${esc(prof.note)}</div>`:''}<div class="person-primary-actions"><button class="btn primary mutating" data-act="new-entry-person" data-type="receivable" data-name="${esc(name)}">💚 أضف فلوس إلي عنده</button><button class="btn secondary mutating" data-act="new-entry-person" data-type="payable" data-name="${esc(name)}">🔴 أضف فلوس إله عندي</button></div><div class="row wrap" style="margin:15px 0"><button class="btn secondary small" data-act="share-person" data-name="${esc(name)}">↗ واتساب</button><button class="btn ghost small" data-act="print-person" data-name="${esc(name)}">🧾 كشف حساب</button><button class="btn ghost small mutating" data-act="person-profile" data-name="${esc(name)}">✎ ملاحظات</button><button class="btn ghost small mutating" data-act="settle-person" data-name="${esc(name)}">⚖ تصفية الحساب</button></div><div class="entries-list">${es.length?es.map((e,i)=>entryCard(e,i)).join(''):empty('ما في تعاملات بعد','')}</div>`,{size:'wide',context:{kind:'person',name}})}
...
async function submitEntry(form){
  ...
  try{
    ...
    await fetchData();
    renderCurrent();
    if(existing)toast('تم تعديل الحركة ✅');
    else{const labels={receivable:`تم تسجيل ${money(amount)} على ${person} ✅`,payable:`تم تسجيل إن عليك ${money(amount)} لـ ${person} ✅`,income:`تم تسجيل دخل ${money(amount)} ✅`,expense:`تم تسجيل مصروف ${money(amount)} ✅`};toast(labels[type]||'تم حفظ الحركة ✅','تراجع',async()=>{await api('entry_delete',{id:savedId});await fetchData();renderCurrent();toast('تم التراجع')})}
  }catch(e){
...
function searchAll(q){q=String(q||'').trim().toLowerCase();if(!q)return [];const out=[];allEntries().forEach(e=>{const hay=`${e.person} ${e.note} ${e.amount} ${e.meta?.reference||''} ${(e.meta?.tags||[]).join(' ')}`.toLowerCase();if(hay.includes(q))out.push({kind:'entry',icon:typeInfo(e.type)[0],title:e.person||typeInfo(e.type)[1],sub:`${typeInfo(e.type)[1]} · ${money(['receivable','payable'].includes(e.type)?e.remaining:e.amount)} · ${humanDate(e.date)}`,id:e.id})});peopleSummary().forEach(p=>{if(p.name.toLowerCase().includes(q))out.push({kind:'person',icon:'👤',title:p.name,sub:p.net>0?`إلك عليه ${money(p.net)}`:p.net<0?`إله عليك ${money(Math.abs(p.net))}`:'الحساب مسكّر',name:p.name})});goals().forEach(d=>{const x=d.data;if(`${x.title} ${x.target} ${x.saved}`.toLowerCase().includes(q))out.push({kind:'goal',icon:x.icon||'🎯',title:x.title,sub:`تحوّش ${money(x.saved||0)} من ${money(x.target||0)}`})});subscriptions().forEach(d=>{const x=d.data;if(`${x.title} ${x.amount}`.toLowerCase().includes(q))out.push({kind:'subscription',icon:'🔁',title:x.title,sub:`${x.direction==='incoming'?'يحّولك':'تدفع'} ${money(x.amount)} · ${humanDate(x.nextDate)}`})});return out.slice(0,14)}
function searchResultsHtml(q){const r=searchAll(q);if(!q.trim())return `<div class="search-hint">اكتب اسم، مبلغ، هدف أو اشتراك.</div>`;if(!r.length)return empty('ما لقينا شيء مطابق','جرّب كلمة ثانية');return r.map(x=>`<button class="global-result" ${x.kind==='entry'?`data-act="edit-entry" data-id="${x.id}"`:x.kind==='person'?`data-act="person-detail" data-name="${esc(x.name)}"`:`data-nav="${x.kind==='goal'?'goals':'subscriptions'}"`}><span>${x.icon}</span><div><b>${esc(x.title)}</b><small>${esc(x.sub)}</small></div><i>←</i></button>`).join('')}
function openSearchHub(){openModal('ابحث بكل شيء','بحث موحد',`<div class="search-modal"><input id="modalGlobalSearch" class="global-search-input" placeholder="اسم، مبلغ، هدف، اشتراك..." autofocus><div id="modalGlobalResults" class="global-search-results"><div class="search-hint">ابدأ بالكتابة…</div></div></div>`,{size:'wide'});setTimeout(()=>$('#modalGlobalSearch')?.focus(),80)}
function onboardingHtml(){return `<div class="onboarding"><div class="onboarding-step"><span>1</span><div><b>سجل اللي صار</b><p>اختار: إلي عند شخص، عليّ لشخص، دخلني مبلغ أو صرفِت مبلغ.</p></div></div><div class="onboarding-step"><span>2</span><div><b>تابع الناس والمواعيد</b><p>كل شخص إله حساب واضح، والمواعيد المتأخرة والقريبة بتطلع قدامك.</p></div></div><div class="onboarding-step"><span>3</span><div><b>كل التفاصيل اختيارية</b><p>المحافظ، الوسوم والتقارير موجودة، بس ما رح نزعجك فيها بالوضع البسيط.</p></div></div><button class="btn primary full" data-act="onboarding-done">ابدأ استخدام تحويشتي ✨</button></div>`}
function maybeShowOnboarding(force=false){if(!force&&state.prefs.onboardingDone)return;setTimeout(()=>openModal('أهلًا في تحويشتي 👋','3 خطوات وبصير كل شيء واضح',onboardingHtml(),{size:'wide'}),450)}
...
document.addEventListener('click',async ev=>{
  ...
  const help=ev.target.closest('[data-help]');if(help){toast(help.dataset.help||'معلومة');return} const b=ev.target.closest('[data-act]');if(!b)return;const a=b.dataset.act,id=b.dataset.id,name=b.dataset.name;
  try{
    if(a==='new-entry')openEntryForm('',{type:b.dataset.type||'receivable'});else if(a==='quick-choice')openQuickChoice();else if(a==='new-entry-person')openEntryForm('',{type:b.dataset.type||'receivable',person:name});else if(a==='edit-entry')openEntryForm(id);else if(a==='delete-entry')await deleteEntry(id);else if(a==='pay-entry')openPaymentForm(id,false);else if(a==='mark-paid')markPaid(id);else if(a==='receipt')await showReceipt(id);else if(a==='share-entry')shareEntry(id);
    ...
    else if(a==='toggle-simple')await savePrefs({simpleMode:!state.prefs.simpleMode});else if(a==='toggle-sounds'){await savePrefs({soundEffects:!state.prefs.soundEffects});if(state.prefs.soundEffects)playSound('success')}else if(a==='replay-onboarding')maybeShowOnboarding(true);else if(a==='onboarding-done'){await savePrefs({onboardingDone:true},false);closeModal();toast('جاهزين نبدأ ✨')}else if(a==='toggle-theme')await savePrefs({theme:state.prefs.theme==='light'?'dark':'light'});else if(a==='toggle-privacy')await savePrefs({hideAmounts:!state.prefs.hideAmounts});else if(a==='toggle-viewonly')await savePrefs({viewOnly:!state.prefs.viewOnly});else if(a==='toggle-card'){const arr=new Set(state.prefs.hiddenCards||[]);arr.has(b.dataset.card)?arr.delete(b.dataset.card):arr.add(b.dataset.card);await savePrefs({hiddenCards:[...arr]})}
...
document.addEventListener('input',ev=>{if(ev.target.id==='globalSearchInput'){const box=$('#globalSearchResults');if(!box)return;const q=ev.target.value;box.classList.toggle('hidden',!q.trim());box.innerHTML=searchResultsHtml(q)}if(ev.target.id==='modalGlobalSearch'){const box=$('#modalGlobalResults');if(box)box.innerHTML=searchResultsHtml(ev.target.value)}});
...
async function init(){
  ...
  try{
    ...
    if('serviceWorker'in navigator){
      navigator.serviceWorker.register('./service-worker.js?v=8.0').catch(err=>console.warn('SW registration failed',err));
    }
    bindTop();
