...
const BUILTIN_CATEGORIES = [
  ...
  ['ترفيه','🎮'],['صحة','🩺'],['تعليم','📚'],['راتب','💼'],['شغل','🧰'],['سفر','✈️'],['أقساط','📆'],['توفير','🎯']
];
...
function validateShell(){
  const required=[
    ...
    '#view-dashboard','#view-transactions','#view-people','#view-goals','#view-planning',
...
function renderCurrent(){
  try{
    ({dashboard:renderDashboard,transactions:renderTransactions,people:renderPeople,goals:renderGoals,planning:renderPlanning,analytics:renderAnalytics,calendar:renderCalendar,settings:renderSettings}[state.view]||renderDashboard)();
  }catch(err){
...
function navigate(view){state.view=view;$$('.view').forEach(v=>v.classList.toggle('active',v.id===`view-${view}`));$$('[data-nav]').forEach(b=>b.classList.toggle('active',b.dataset.nav===view));$('#pageTitle').textContent=({dashboard:'لوحة التحكم',transactions:'كل الحركات',people:'الأشخاص',goals:'أهداف التحويش',planning:'التخطيط المالي',analytics:'الإحصائيات',calendar:'الرزنامة المالية',settings:'الإعدادات'})[view]||'تحويشتي';renderCurrent();window.scrollTo({top:0,behavior:'smooth'})}
